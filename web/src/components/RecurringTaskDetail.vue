<template>
  <div class="task-detail-page">
    ...
    <div class="main-content">
      <div class="detail-container">
        ...
        <div class="task-details-section">
          ...
          <!-- Recurring Task Specific Info -->
          <div v-if="task.task_type === 'repeat'" class="task-specific-info">
            <div class="section-header">
              <h3>当前周待办事项</h3>
              <button class="manage-button" @click="showExecutionModal = true">
                管理待办事项
              </button>
            </div>
            <div class="executions-list">
              <div 
                v-for="execution in weeklyExecutions" 
                :key="execution.id" 
    ...
    <!-- Execution Management Modal -->
    <div v-if="showExecutionModal" class="modal-overlay" @click="showExecutionModal = false">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>管理待办事项</h3>
          <button class="close-button" @click="showExecutionModal = false">×</button>
        </div>
        <div class="modal-body">
          <div class="executions-list">
            <div 
              v-for="execution in allExecutions" 
              :key="execution.id" 
              class="execution-item"
            >
              <div class="execution-header">
...
export default {
  ...
  setup() {
    ...
    // 计算当前周的待办事项
    const weeklyExecutions = computed(() => {
      const now = new Date();
      ...
      return allExecutions.value.filter(execution => {
        const executionDate = new Date(execution.execution_date);
        return executionDate >= startOfWeek && executionDate < endOfWeek;
      });
    });
    ...
    const loadExecutions = async () => {
      try {
        ...
        allExecutions.value = response.items;
      } catch (error) {
